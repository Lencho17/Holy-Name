...
/**
 * Simple in-memory cache for aggregated site content, keyed by schoolId
 */
let contentCache = {};
const CACHE_TTL = 30 * 1000; // 30 seconds

const getAggregatedContent = async (schoolId) => {
  // Build queries dynamically based on schoolId
  const applySchoolFilter = (query) => {
    if (schoolId) {
  ...
  const [
    ...
    applySchoolFilter(supabase.from('site_settings').select('*')).limit(1).maybeSingle(),
    applySchoolFilter(supabase.from('notices').select('*')).order('created_at', { ascending: false }),
    applySchoolFilter(supabase.from('gallery').select('*')).order('created_at', { ascending: false }),
    applySchoolFilter(supabase.from('events').select('*')).order('created_at', { ascending: false }),
    applySchoolFilter(supabase.from('highlights').select('*')).order('created_at', { ascending: false }),
    applySchoolFilter(supabase.from('faculty').select('*')).order('order_index', { ascending: true }),
    applySchoolFilter(supabase.from('alumni').select('*')).order('created_at', { ascending: false }),
    applySchoolFilter(supabase.from('stats').select('*')).order('created_at', { ascending: true }),
    applySchoolFilter(supabase.from('faqs').select('*')).order('order_index', { ascending: true }),
    applySchoolFilter(supabase.from('courses').select('*')).order('created_at', { ascending: true }),
    applySchoolFilter(supabase.from('messages').select('*')),
    applySchoolFilter(supabase.from('emeritus').select('*')).order('order_index', { ascending: true }),
    applySchoolFilter(supabase.from('center_of_excellence').select('*')).order('order_index', { ascending: true })
  ]);
...
router.get('/', optionalProtect, async (req, res) => {
  try {
    ...
    if (contentCache[cacheKey] && (now - contentCache[cacheKey].lastFetched < CACHE_TTL)) {
      return res.json(contentCache[cacheKey].data);
    }

    const aggregatedContent = await getAggregatedContent(schoolId);
...
router.put('/', protect, async (req, res) => {
  try {
    ...
    if (Object.keys(cleanSettings).length > 0) {
      // Get the settings row if it exists for this school
      let existingQuery = supabase.from('site_settings').select('id').limit(1);
      if (req.user.school_id) {
        existingQuery = existingQuery.eq('school_id', req.user.school_id);
      } else {
      ...
      if (existing) {
        ...
        cleanSettings.school_id = req.user.school_id || null;
    ...
    for (const mod of syncModules) {
      if (updateData[mod.key]) {
        // Delete all existing for this module using a robust timestamp-based range filter for this school
        let deleteQuery = supabase.from(mod.table).delete().gte('created_at', '1900-01-01');
        if (req.user.school_id) {
          deleteQuery = deleteQuery.eq('school_id', req.user.school_id);
        } else {
        ...
        if (delError) {
          ...
          let fallbackDelete = supabase.from(mod.table).delete().not('id', 'is', null);
          if (req.user.school_id) fallbackDelete = fallbackDelete.eq('school_id', req.user.school_id);
          else fallbackDelete = fallbackDelete.is('school_id', null);
    ...
    if (updateData.coursesPage?.courses && Array.isArray(updateData.coursesPage.courses)) {
      let delQuery = supabase.from('courses').delete().neq('id', '00000000-0000-0000-0000-000000000000');
      if (req.user.school_id) delQuery = delQuery.eq('school_id', req.user.school_id);
      else delQuery = delQuery.is('school_id', null);
      await delQuery;
    ...
    // Fetch updated content for the current user's school context
    // The protect middleware sets req.user.school_id
    const aggregatedContent = await getAggregatedContent(req.user.school_id || null);
    res.json(aggregatedContent);
...
router.clearCache = (schoolId) => {
  if (schoolId) {
    delete contentCache[schoolId];
  } else {
    contentCache = {};
  }
};
