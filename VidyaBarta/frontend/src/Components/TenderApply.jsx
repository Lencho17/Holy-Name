...
function TenderApply() {
  ...
  const handleDownloadReceipt = async () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(20);
    doc.text(schoolProfile?.name?.toUpperCase() || (schoolProfile?.name?.toUpperCase() || "OUR SCHOOL"), 115, 16, { align: "center" });
    ...
    doc.text(`${schoolProfile?.name || "Our School"} | Tender Cell | ${schoolProfile?.phone || "N/A"}`, 105, 284, { align: "center" });
          ...
          <form onSubmit={handleSubmit} className="p-5 md:p-12 space-y-8 md:space-y-12">
            ...
            <div className="pt-10 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
              <p className="text-xs text-gray-500 font-bold uppercase tracking-widest max-w-md text-center md:text-left">
                By submitting this bid, you agree to ${schoolProfile?.name || "Our School"}'s procurement terms.
              </p>
              <button 
                type="submit" 
                disabled={submitting}
