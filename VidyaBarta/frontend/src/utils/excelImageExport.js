...
export const exportStudentMarksheetToExcel = async ({
  ...
  student,
  examName,
  data, // contains subjects, scores, totals, grades, rank, promotion
  filename
}) => {
  const workbook = new ExcelJS.Workbook();
  ...
  const schoolName = (schoolProfile?.name || 'HOLY NAME HIGHER SECONDARY SCHOOL').toUpperCase();
  const schoolAddress = schoolProfile?.officeAddress || schoolProfile?.address || 'Sivasagar, Assam - 785640';
  const totalCols = marksheetType === 'combined' ? 9 : 7;
  ...
  const r3 = worksheet.addRow([`${examName.toUpperCase()} - STUDENT PROGRESS MARKSHEET`]);
  worksheet.mergeCells(3, 1, 3, totalCols);
  r3.font = { name: 'Arial', size: 12, bold: true, color: { argb: 'FF1E3A8A' } };
  r3.alignment = { horizontal: 'center', vertical: 'middle' };
  r3.height = 20;
  ...
  if (marksheetType === 'unit_test') {
    ...
    // Annual
    tableHeaders = ['Sl', 'Subject Name', 'UT Average', 'Terminal Average', 'Final Score (/100)', 'Annual Grade', 'Remarks'];
  }
  ...
  sumRow1.font = { name: 'Arial', size: 10, bold: true, color: { argb: 'FF166534' } };

  const sumRow2 = worksheet.addRow([
    `Class Rank: ${data.rank || '—'}`, '',
    `Result: ${data.status || 'PASSED'}`, '',
    marksheetType === 'annual' ? `Promotion: ${data.promotion || 'PROMOTED'}` : `Attendance: ${data.attendance || '95%'}`
  ]);
