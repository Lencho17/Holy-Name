...
function Header() {
  ...
  const dropdownLinkClass = ({ isActive }) =>
    `flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
      isActive ? "bg-canva-purple/10 text-canva-purple" : "text-slate-600 hover:bg-slate-50 hover:text-canva-purple"
    }`;
    ...
    <header className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto flex justify-between items-center w-full px-4 md:px-6 py-3">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="hidden md:flex items-center text-[10px] text-slate-400 gap-1 border-l border-slate-300 pl-2">
                <span className="material-symbols-outlined text-[12px]">call</span> 6901055733
              </span>
            </div>
          </div>
        </Link>
        ...
        <nav className="hidden xl:flex items-center gap-1">
          ...
          >
            <button className={`flex items-center gap-1 px-4 py-2 font-semibold transition-all duration-300 rounded-full text-sm text-slate-600 hover:bg-slate-100 hover:text-slate-900 ${showMore ? 'bg-slate-100' : ''}`}>
              More <span className={`material-symbols-outlined text-sm transition-transform duration-300 ${showMore ? 'rotate-180' : ''}`}>expand_more</span>
            </button>
              ...
              <div className="absolute top-full right-0 pt-2 w-48">
                <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-2 flex flex-col gap-1">
                  <NavLink to="/faculty" className={dropdownLinkClass}>
                    <span className="material-symbols-outlined text-[18px]">group</span> Faculty
                  </NavLink>
                  <NavLink to="/gallery" className={dropdownLinkClass}>
                    <span className="material-symbols-outlined text-[18px]">gallery_thumbnail</span> Gallery
                  </NavLink>
                  <NavLink to="/career" className={dropdownLinkClass}>
                    <span className="material-symbols-outlined text-[18px]">work</span> Career
                  </NavLink>
                  <NavLink to="/complaints" className={dropdownLinkClass}>
                    <span className="material-symbols-outlined text-[18px]">lightbulb</span> Suggestions
                  </NavLink>
                  <NavLink to="/contact" className={dropdownLinkClass}>
                    <span className="material-symbols-outlined text-[18px]">contact_support</span> Contact
                  </NavLink>
                </div>
              </div>
            )}
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            ...
            title="Call Us Now"
          >
            <span className="material-symbols-outlined text-[18px] md:text-[20px] group-hover:animate-pulse">call</span>
          </a>
          <button
      ...
      >
        <ul className="flex flex-col font-medium py-4 px-6 gap-2">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">More Details</p>
          <li><NavLink to="/faculty" className={dropdownLinkClass} onClick={() => setShowMediaIcons(false)}>Faculty</NavLink></li>
          <li><NavLink to="/gallery" className={dropdownLinkClass} onClick={() => setShowMediaIcons(false)}>Gallery</NavLink></li>
          <li><NavLink to="/career" className={dropdownLinkClass} onClick={() => setShowMediaIcons(false)}>Career</NavLink></li>
          <li><NavLink to="/complaints" className={dropdownLinkClass} onClick={() => setShowMediaIcons(false)}>Suggestions</NavLink></li>
          <li><NavLink to="/contact" className={dropdownLinkClass} onClick={() => setShowMediaIcons(false)}>Contact</NavLink></li>
          <div className="h-px bg-slate-100 my-2 w-full"></div>
