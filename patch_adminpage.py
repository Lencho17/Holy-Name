...
if 'FiChevronDown' not in content:
...
if 'const SidebarItem = ({ active' not in content:
    content = content.replace("function AdminPage() {", sidebar_components + "\nfunction AdminPage() {")
...
main_str = """<main className="flex-1 overflow-y-auto p-2 sm:p-4 md:p-8" style={{ backgroundColor: '#F8FAFC' }}>"""

start_index = content.find(return_start_str)
end_index = content.find(main_str)

if start_index != -1 and end_index != -1:
    new_layout = """  const getInitials = (name) => {
    if (!name) return 'SA';
...
"""
    content = content[:start_index] + new_layout + content[end_index + len(main_str):]
    ...
    with open(file_path, 'w') as f:
        f.write(content)
    print("Patched AdminPage.jsx successfully.")
else:
