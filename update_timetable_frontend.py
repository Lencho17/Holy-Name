...
const TimetableManager = ({ apiUrl, token }) => {
      ...
      periodColumns.forEach(col => {
        ...
        if (cell && cell.staff_id) {
          const clash = allTimetables.find(t => 
            ...
            !(t.class_level === editingClass.class_level && t.section === (editingClass.section || 'A'))
          );
          if (clash) {
             clashes.push({
               day: row.day,
  ...
  const generatePDF = (cls) => {
    const clsTimetable = allTimetables.filter(t => t.class_level === cls.class_level && t.section === (cls.section || 'A'));
    if (clsTimetable.length === 0) return alert("No timetable data to generate PDF.");

    const doc = new jsPDF('landscape');
    doc.setFontSize(18);
    doc.text(`Class ${cls.class_level} - ${cls.section || 'A'} Timetable`, 14, 20);
    
    // Determine max period for this class
    let maxP = 1;
    ...
    doc.save(`Timetable_Class_${cls.class_level}_${cls.section || 'A'}.pdf`);
  };
  ...
  const getClassStatus = (cls) => {
    const clsTimetable = allTimetables.filter(t => t.class_level === cls.class_level && t.section === (cls.section || 'A'));
    if (clsTimetable.length === 0) return { label: 'Not Created', color: 'bg-gray-100 text-gray-500' };
    const isPublished = clsTimetable.some(t => t.is_published);
